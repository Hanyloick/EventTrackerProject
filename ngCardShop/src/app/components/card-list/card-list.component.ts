...
export class CardListComponent implements OnInit {
  ...
  loadAllCards() {
    this.cardService.index().subscribe({
      ...
      error: (card) => {
        console.error(
          'CardListComponent.LoadCards(): error loading Cards:'
        );
        console.error(card);
      },
    });
  ...
  updateTodo(editCard: Card) {
    this.cardService.update(editCard).subscribe({
      ...
      error: (nojoy) => {
        console.error(
          'CardListComponent.UpdateCard(): error Updating Card:'
        );
        console.error(nojoy);
      },
    });
  ...
  deleteCard(id: number) {
    this.cardService.delete(id).subscribe({
      ...
      error: (nojoy) => {
        console.error('TodoListHttpComponent.DeleteCard(): error Deleting Card:');
        console.error(nojoy);
      },
    });
  }
